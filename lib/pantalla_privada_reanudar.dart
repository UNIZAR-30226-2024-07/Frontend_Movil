...
class PrivateScreenResume extends StatefulWidget {
  //final String idMesa;
  final User user;
  ...
  PrivateScreenResume(this.boardId, this.user, {super.key});

  @override
  _PrivateScreenResumeState createState() => _PrivateScreenResumeState();
}

class _PrivateScreenResumeState extends State<PrivateScreenResume> {
  ...
  Future emitEntrar() async {
    if (!widget.hecho) {
      try {
        ...
        widget.socket?.emit('resume private board', body);
      }catch (err) {
        if (true) {
          print(err);
        }
